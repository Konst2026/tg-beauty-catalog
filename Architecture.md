...
export interface IBookingRepository {
  ...
  cancel(id: string, clientTelegramId: number): Promise<Booking | null>;
}
...
// src/domain/schedule/slot-calculator.ts — чистая функция, без I/O
...
#### Реализованные эндпоинты (Этап 1, в продакшене)

```
# Публичные (без авторизации)
GET  /api/v1/catalog/masters                     ?category_id=&city=&available_today=
GET  /api/v1/catalog/masters/:id                 профиль + активные услуги
GET  /api/v1/catalog/masters/:id/slots           ?date=YYYY-MM-DD&service_id=UUID
GET  /health

# Клиент (X-Init-Data: <tg initData>)
POST   /api/v1/bookings                          { master_id, service_id, start_time, end_time, price_snapshot }
GET    /api/v1/bookings/mine                     записи текущего клиента
DELETE /api/v1/bookings/:id                      отмена своей записи

# Мастер (X-Init-Data: <tg initData> — master_id берётся из БД, не из body)
GET    /api/v1/me/services                       список услуг
POST   /api/v1/me/services                       создать услугу (лимит 50)
PUT    /api/v1/me/services/:id                   обновить услугу
DELETE /api/v1/me/services/:id                   удалить услугу

GET    /api/v1/me/schedule/template              недельный шаблон (7 дней)
PUT    /api/v1/me/schedule/template              { day_of_week, is_working, start_time, end_time }
GET    /api/v1/me/schedule/overrides             ?from=YYYY-MM-DD&to=YYYY-MM-DD
PUT    /api/v1/me/schedule/overrides             { override_date, is_working, start_time?, end_time?, reason? }
DELETE /api/v1/me/schedule/overrides/:id         удалить переопределение даты
```

#### Запланированные эндпоинты (следующие итерации)

```
# Мастер — бот и уведомления
POST   /api/v1/me/bot/connect                    { bot_token } → getMe → setWebhook
GET    /api/v1/me/calendar                       ?from=&to= (schedule + overrides + bookings)
POST   /api/v1/me/calendar/block
DELETE /api/v1/me/calendar/block/:id

# Webhook (multi-bot)
POST   /webhook/tg/:token_hash                   диспетчер для N ботов мастеров

# Этап 2–4 (AI, CRM, монетизация)
...
**Безопасность:** `X-Init-Data` проверяется HMAC-SHA256 на каждый запрос. `master_id` берётся только из `masters.findByTelegramId(user.id)`, никогда из тела запроса.

### 6.6 Composition Root — единственное место, где сходятся все слои
...
```typescript
// src/app.ts — ЕДИНСТВЕННЫЙ Composition Root (актуальное состояние)

// ── Repos (реализации Port-интерфейсов) ────────────────────────────────
...
const createService     = new CreateServiceUseCase(servicesRepo);   // лимит 50
...
const upsertSchedule    = new UpsertScheduleUseCase(schedulesRepo); // валидация start<end
...
```
GET /api/v1/catalog/masters/:id/slots?date=2026-06-15&service_id=UUID

1. Найти услугу по service_id → взять duration_min
2. Проверить schedule_overrides для этой даты
   - is_working = false → вернуть [] (выходной/отпуск)
   - is_working = true + время → использовать это время
   - Нет override → взять schedules для day_of_week (0=Sun..6=Sat)
3. Если нет рабочих часов → вернуть []
4. Загрузить все confirmed+pending бронирования мастера на эту дату
5. calculateSlots() — чистая функция domain/schedule/slot-calculator.ts:
   - Итерирует от workStart до workEnd шагом duration_min
   - Пропускает слоты, пересекающиеся с bookedRanges
6. Вернуть свободные слоты [{ start: ISO, end: ISO }]
```
...
#### Реализовано (Этап 1 частично — коммит a5a72bf, задеплоено на Railway)
...
│   │   │   └── master.entity.ts
│   │   ├── service/
│   │   │   └── service.entity.ts      # Service, CreateServiceInput, UpdateServiceInput
│   │   ├── schedule/
│   │   │   ├── schedule.entity.ts     # Schedule, ScheduleOverride, UpsertScheduleInput
│   │   │   └── slot-calculator.ts     # calculateSlots() — чистая функция, нет I/O
│   │   └── ports/
│   │       ├── booking.repo.port.ts   # IBookingRepository
│   │       ├── master.repo.port.ts    # IMasterRepository
│   │       ├── service.repo.port.ts   # IServiceRepository
│   │       └── schedule.repo.port.ts  # IScheduleRepository
│   │
│   ├── use-cases/
│   │   ├── create-booking/            # + test (4 unit-теста)
│   │   ├── cancel-booking/            # + test (3 unit-теста)
│   │   ├── get-masters/
│   │   ├── get-master-by-id/
│   │   ├── get-my-bookings/
│   │   ├── get-services/
│   │   ├── create-service/            # бизнес-правило: лимит 50 услуг
│   │   ├── update-service/
│   │   ├── delete-service/
│   │   ├── get-schedule/
│   │   ├── upsert-schedule/           # валидация start_time < end_time
│   │   ├── get-overrides/
│   │   ├── upsert-override/
│   │   ├── delete-override/
│   │   └── get-available-slots/       # override > weekly template → calculateSlots()
│   │
│   ├── adapters/
│   │   ├── http/
│   │   │   ├── catalog/
│   │   │   │   └── catalog.controller.ts   # GET /catalog/masters, /:id, /:id/slots
│   │   │   ├── bookings/
│   │   │   │   └── bookings.controller.ts  # POST, GET /mine, DELETE /:id
│   │   │   ├── services/
│   │   │   │   └── services.controller.ts  # GET/POST/PUT/DELETE /me/services
│   │   │   └── schedule/
│   │   │       └── schedule.controller.ts  # /me/schedule/template, /me/schedule/overrides
│   │   └── repositories/
│   │       ├── postgres-masters.repo.ts
│   │       ├── postgres-bookings.repo.ts
│   │       ├── postgres-services.repo.ts
│   │       └── postgres-schedules.repo.ts  # UPSERT ON CONFLICT, booked ranges query
│   │
│   ├── infrastructure/
│   │   ├── auth/
│   │   │   └── telegram-auth.ts
...
│   │   └── supabase/
│   │       └── migrations/
│   │           ├── 001_initial.sql    # основная схема (masters, services, schedules, bookings…)
│   │           ├── 002_rls.sql
│   │           └── 003_bot_token_hash.sql
│   │
│   └── shared/
│       ├── config/env.ts              # Zod-валидация env vars при старте
│       ├── errors/domain-error.ts
│       └── lib/
│           ├── telegram-auth.ts       # verifyInitData() HMAC-SHA256
│           └── telegram-auth.test.ts  # 6 unit-тестов
│
├── app.ts          # Composition Root: repos → use cases → routes → error handler
├── server.ts
├── migrate.js      # Idempotent migrations с _migrations tracking table
├── Dockerfile      # Двухэтапная сборка
├── railway.toml    # healthcheck /health
└── .env.example
```

**Деплой:** https://backend-production-ed11d.up.railway.app  
**БД:** Supabase PostgreSQL 17.6 (Transaction pooler порт 6543, Session pooler порт 5432)
